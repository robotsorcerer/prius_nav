...
def high_res_colormap(low_res_cmap, resolution=1000, max_value=1):
    ...
    high_res = np.stack(
        [np.interp(new_x, x, low_res[:, i]) for i in range(low_res.shape[1])],
        axis=1
    )
...
def log_output_tensorboard(writer, prefix, index, suffix, n_iter, depth, disp, warped, diff, mask):
    ...
    writer.add_image(
        '{} Dispnet Output Normalized{}/{}'.format(prefix, suffix, index),
        disp_to_show, n_iter
    )
    writer.add_image(
        '{} Depth Output Normalized{}/{}'.format(prefix, suffix, index),
        depth_to_show, n_iter
    )
    ...
    for j, (warped_j, diff_j) in enumerate(zip(warped, diff)):
        ...
        writer.add_image(
            '{} Warped Outputs {}'.format(prefix, whole_suffix),
            warped_to_show, n_iter
        )
        ...
        if mask is not None:
            ...
            writer.add_image(
                '{} Exp mask Outputs {}'.format(prefix, whole_suffix),
                mask_to_show, n_iter
            )
