...
def conv(in_planes, out_planes, kernel_size=3):
    ...
    return nn.Sequential(
        nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, padding=padding, stride=2),
...
class PoseExpNet(nn.Module):
    """
    Neural network model that simultaneously learns pose transformation
    between images and the "explanation mask", which is essentially predicting
    'soft keypoints' in images, in order to enhance robustness to occlusion and
    uncontrolled motion.
    """
    ...
    def __init__(self, nb_ref_imgs=2, output_exp=False):
        ...
        if self.output_exp:
            ...
            nb_ref_imgs = self.nb_ref_imgs
            self.pred_mask4 = nn.Conv2d(upconv_planes[1], nb_ref_imgs, kernel_size=3, padding=1)
            self.pred_mask3 = nn.Conv2d(upconv_planes[2], nb_ref_imgs, kernel_size=3, padding=1)
            self.pred_mask2 = nn.Conv2d(upconv_planes[3], nb_ref_imgs, kernel_size=3, padding=1)
            self.pred_mask1 = nn.Conv2d(upconv_planes[4], nb_ref_imgs, kernel_size=3, padding=1)
    ...
    def forward(self, target_image, ref_imgs):
        ...
        if self.output_exp:
            ...
            exp_mask4 = sigmoid(self.pred_mask4(out_upconv4))
            exp_mask3 = sigmoid(self.pred_mask3(out_upconv3))
            exp_mask2 = sigmoid(self.pred_mask2(out_upconv2))
            exp_mask1 = sigmoid(self.pred_mask1(out_upconv1))
        else:
            exp_mask4 = None
            exp_mask3 = None
            exp_mask2 = None
