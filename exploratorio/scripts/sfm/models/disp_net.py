...
def downsample_conv(in_planes, out_planes, kernel_size=3):
    ...
    return nn.Sequential(
        nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=2, padding=padding),
        nn.ReLU(inplace=True),
        nn.Conv2d(out_planes, out_planes, kernel_size=kernel_size, padding=padding),
        nn.ReLU(inplace=True)
    )
...
class DispNetS(nn.Module):
    """
    Neural network model that will be used to predict depth from RGB images.
    """
